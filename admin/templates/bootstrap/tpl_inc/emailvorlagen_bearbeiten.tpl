...
<div id="content">
    <form name="vorlagen_aendern" method="post" action="{$shopURL}{$route}" enctype="multipart/form-data">
        {$jtl_token}
                        ...
                        {if $mailTemplate->getAttachments($kSprache)|count > 0}
                            <div class="row mt-4">
                                ...
                                <div class="col-sm-auto">
                                    <a href="{$shopURL}{$route}?kEmailvorlage={$mailTemplate->getID()}&kS={$kSprache}&a=pdfloeschen&token={$smarty.session.jtl_token}{if $mailTemplate->getPluginID() > 0}&kPlugin={$mailTemplate->getPluginID()}{/if}"
