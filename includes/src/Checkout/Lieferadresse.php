...
class Lieferadresse extends Adresse
{
    ...
    public static function createFromShippingAddress(?array $post = null): Lieferadresse
    {
        ...
        $shippingAddress->cAnrede         = $customer->getSalutation();
        $shippingAddress->cVorname        = $customer->getFirstName();
        $shippingAddress->cNachname       = $customer->getName();
        $shippingAddress->cStrasse        = $customer->getStreet();
        $shippingAddress->cHausnummer     = $customer->getStreetNumber();
        $shippingAddress->cPLZ            = $customer->getZipCode();
        $shippingAddress->cOrt            = $customer->getCity();
        $shippingAddress->cLand           = $customer->getCountry();
        $shippingAddress->cMail           = $customer->getEmail();
        $shippingAddress->cTel            = $customer->getPhoneNumber();
        $shippingAddress->cFax            = $customer->getFaxNumber();
        $shippingAddress->cFirma          = $customer->getCompany();
        $shippingAddress->cZusatz         = $customer->getAdditionalInformation();
        $shippingAddress->cTitel          = $customer->getTitle();
        $shippingAddress->cAdressZusatz   = $customer->getAdditionalAddressInformation();
        $shippingAddress->cMobil          = $customer->getMobilePhoneNumber();
        $shippingAddress->cBundesland     = $customer->getState();
        $shippingAddress->angezeigtesLand = LanguageHelper::getCountryCodeByCountryName($shippingAddress->cLand);
