...
{if isset($wishlistItems)}
    <section class="panel panel-default box box-wishlist" id="sidebox{$oBox->kBox}">
        ...
        <div class="panel-body">
            ...
            <hr>
            <a href="{get_standard_link id='jtl.php'}?wl={if isset($Boxen.Wunschliste->CWunschlistePos_arr)}{$Boxen.Wunschliste->CWunschlistePos_arr[0]->kWunschliste}{else}{$oBox->CWunschlistePos_arr[0]->kWunschliste}{/if}" class="btn btn-default btn-block btn-sm">{lang key="goToWishlist" section="global"}</a>
        </div>
    </section>
{/if}  