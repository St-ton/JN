...
{/if}
<div id="result-wrapper_buy_form_{$Artikel->kArtikel}" class="product-cell clearfix{if $Einstellungen.template.productlist.hover_productlist === 'Y'} hover-enabled{/if}{if isset($listStyle) && $listStyle === 'list'} active{/if}">
    <div class="product-body row {if $tplscope !== 'list'} text-center{/if}">
                    ...
                    <div class="actions btn-group btn-group-xs btn-group-justified" role="group" aria-label="...">
                        {block name='product-actions'}
                            {if !($Artikel->nIstVater && $Artikel->kVaterArtikel === 0)}
                                {if $Einstellungen.artikeluebersicht.artikeluebersicht_vergleichsliste_anzeigen === 'Y'}
                                    <button name="Vergleichsliste" type="submit" class="compare badge" data-toggle="tooltip" data-placement="top" title="{lang key='addToCompare' section='productOverview'}">
                                        <img class="svg" src="{$imageBaseURL}gfx/compare.svg" alt="{$alt}" />
                                    </button>
                                {/if}
                                {if $Einstellungen.global.global_wunschliste_anzeigen === 'Y' && $Einstellungen.artikeluebersicht.artikeluebersicht_wunschzettel_anzeigen === 'Y'}
                                    <button name="Wunschliste" type="submit" class="wishlist badge" data-toggle="tooltip" data-placement="top" title="{lang key='addToWishlist' section='productDetails'}">
                                        <img class="svg" src="{$imageBaseURL}gfx/wishlist.svg" alt="{$alt}" />
                                    </button>
                                {/if}
                            ...
                            {if $Einstellungen.template.productlist.quickview_productlist === 'Y' && !$Artikel->bHasKonfig}
                                <button class="quickview badge hidden-xs" data-src="{$Artikel->cURLFull}" data-target="buy_form_{$Artikel->kArtikel}" data-toggle="tooltip" data-placement="top" title="{lang key='downloadPreview' section='productDownloads'}">
        ...
        <div class="col-xs-4 product-detail">
            <div class="product-detail-cell text-right" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
                <link itemprop="businessFunction" href="http://purl.org/goodrelations/v1#Sell" />
                {block name='form-basket'}
                    {include file='productdetails/price.tpl' Artikel=$Artikel tplscope=$tplscope}
                    <div class="delivery-status top15">
