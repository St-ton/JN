            ...
            {if !empty($links)}
                {block name='snippets-linkgroup-recursive-lis'}
                    {foreach $links as $li}
                        ...
                        {/if}
                        <li class="mb-2 {if $li->getChildLinks()->count() > 0 && isset($dropdownSupport)}dropdown dropdown-multi{/if}{if $li->getIsActive() || (isset($activeParent) && $activeParent == $li->getID())} active{/if}">
