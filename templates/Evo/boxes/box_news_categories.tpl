        ...
        <div class="box-body dropdown">
            <ul class="nav nav-list">
                {foreach name=newskategorie from=$oBox->oNewsKategorie_arr item=oNewsKategorie}
                    <li><a href="{$oNewsKategorie->cURL}" title="{$oNewsKategorie->cName}">{$oNewsKategorie->cName} <span class="badge pull-right">{$oNewsKategorie->nAnzahlNews}</span></a></li>
                {/foreach}
            </ul>
        </div>
    </section>
    ...
    <section class="panel panel-default box box-newscategories" id="sidebox{$oBox->kBox}">
       ...
       <div class="panel-body">
          ...
          {foreach name=newskategorie from=$Boxen.NewsKategorie->oNewsKategorie_arr item=oNewsKategorie}
             <li><a href="{$oNewsKategorie->cURL}" title="{$oNewsKategorie->cBeschreibung|escape:"html"}" class="tooltip">{$oNewsKategorie->cName}</a> <span class="badge pull-right">{$oNewsKategorie->nAnzahlNews}</span></li>
          {/foreach}
          </ul>
       </div>
    </section>
