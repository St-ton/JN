 ...
 * @return bool
 */
function schalteBewertungFrei($reviewIDs, $productIDs, $allRatings): bool
{
    ...
    if (is_array($reviewIDs) && count($reviewIDs) > 0) {
        ...
        foreach ($reviewIDs as $i => $reviewID) {
            ...
            $idx       = array_search($reviewID, $allRatings);
            $productID = $idx !== false ? $productIDs[$idx] : $productIDs[$i];
            ...
            Shop::Container()->getDB()->query(
                'UPDATE tbewertung
                    SET nAktiv = 1
                    WHERE kBewertung = ' . $reviewID,
                ReturnType::AFFECTED_ROWS
            );
            aktualisiereDurchschnitt($productID, $conf['bewertung']['bewertung_freischalten']);
            checkeBewertungGuthabenBonus($reviewID, $conf);
            $tags[] = CACHING_GROUP_ARTICLE . '_' . $productID;
        }
