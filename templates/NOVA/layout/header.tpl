    ...
    {block name='layout-header-content-all-starttags'}
        {block name='layout-header-content-wrapper-starttag'}
            <div id="content-wrapper"
                 class="{if $Einstellungen.template.theme.left_sidebar === 'Y' || $smarty.const.PAGE_ARTIKELLISTE === $nSeitenTyp}is-item-list container-fluid container-fluid-xl{/if}
                        {if $isFluidBanner || $isFluidSlider} has-fluid{/if}">
        {/block}

        {block name='layout-header-breadcrumb'}
            {container fluid=($Einstellungen.template.theme.left_sidebar === 'Y' || $smarty.const.PAGE_ARTIKELLISTE === $nSeitenTyp || (isset($Link) && $Link->getIsFluid())) class="breadcrumb-container"}
        ...
        {if !$bExclusive && !empty($boxes.left|strip_tags|trim) && ($Einstellungen.template.theme.left_sidebar === 'Y' || $smarty.const.PAGE_ARTIKELLISTE === $nSeitenTyp)}
