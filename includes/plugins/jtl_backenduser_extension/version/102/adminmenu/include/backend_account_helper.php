...
class BackendAccountHelper
{
     ...
     * @return string
     */
    private function getFullImageURL($relURL)
    {
        return (method_exists(Shop::class, 'getImageBaseURL')
                ? Shop::getImageBaseURL()
                : Shop::getURL() . '/') . ltrim($relURL, '/');
    }
                ...
                if (isset($author->kAdminlogin) && $author->kAdminlogin > 0) {
                    ...
                    if (isset($author->extAttribs['useAvatar']) && $this->getConfigParam('use_avatar', 'N') === 'Y') {
                        ...
                        if ($author->extAttribs['useAvatar']->cAttribValue === 'U') {
                            ...
                            $author->cAvatarImgSrcFull = $this->getFullImageURL($author->extAttribs['useAvatarUpload']->cAttribValue);
                        }
