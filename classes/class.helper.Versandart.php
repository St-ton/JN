    ...
    public static function versandzahlungsartGueltig($Zahlungsart)
    {
        ...
        switch ($Zahlungsart->cModulId) {
            ...
            case 'za_clickpay_dd_jtl':
                ...
                break;
            case 'za_clickandbuy_jtl':
                if (!pruefeZahlungsartMinBestellungen($Zahlungsart->einstellungen['zahlungsart_ueberweisung_min_bestellungen'])) {
                    return false;
                }

                if (!pruefeZahlungsartMinBestellwert($Zahlungsart->einstellungen['zahlungsart_clickandbuy_min'])) {
                    return false;
                }

                if (!pruefeZahlungsartMaxBestellwert($Zahlungsart->einstellungen['zahlungsart_clickandbuy_max'])) {
                    return false;
                }
                break;
            case 'za_eos_cc_jtl':
            ...
            case 'za_postfinance_jtl':
                ...
                break;
            case 'za_saferpay_jtl':
                if (!pruefeZahlungsartMinBestellungen($Zahlungsart->einstellungen['zahlungsart_saferpay_min_bestellungen'])) {
                    return false;
                }

                if (!pruefeZahlungsartMinBestellwert($Zahlungsart->einstellungen['zahlungsart_saferpay_min'])) {
                    return false;
                }

                if (!pruefeZahlungsartMaxBestellwert($Zahlungsart->einstellungen['zahlungsart_saferpay_max'])) {
                    return false;
                }
                break;
            case 'za_safetypay':
                    ...
                    if ($cLandAttr && ($cLand == $cLandAttr || $bCheckLieferadresse == false)) {
                        ...
                        foreach ($arrKosten as $staffel) {
                            ...
                            if ($fPreis >= 0 && $bisAnzahl > 0 && $nAnzahl <= $bisAnzahl) {
                                ...
                                if ($netPricesActive === true) {
                                    $oVersandPos->cPreisLocalized = gibPreisStringLocalized(berechneNetto(floatval($oVersandPos->fKosten), $steuerSatz)) . ' ' . Shop::Lang()->get('plus', 'productDetails') . ' ' . Shop::Lang()->get('vat', 'productDetails');
                                } else {
