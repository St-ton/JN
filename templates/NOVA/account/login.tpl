{block name='account-login'}
    ...
    {if $showTwoFAForm|default:false}
        {row class="login-form"}
            {col sm=8 lg=6}
                {form id="twofa_form" action="{get_static_route id='jtl.php'}" method="post" role="form" class="jtl-validate" slide=true}
                    <fieldset>
                        {include file='snippets/form_group_simple.tpl'
                            options=['text', 'inputTwoFA', 'TwoFA_code', null, {lang key='Two factor auth code'}, true, null, 'off']
                        }
