...
<div class="well well-sm">
    <form id="frm_filter" name="frm_filter" action="{get_standard_link id='news.php'}" method="post" class="form-inline text-center">
        {$jtl_token}
            ...
            <div class="col-xs-7 col-md-8 col-lg-9">
                <ul class="pagination">
                    {if $oBlaetterNavi->nAktuelleSeite > 1}
                        <li>
                            <a href="{get_standard_link id='news.php'}?s={$oBlaetterNavi->nVoherige}">&laquo; {lang key="previous" section="productOverview"}</a>
                        </li>
                    {/if}
                    {if $oBlaetterNavi->nAnfang != 0}
                        <li><a href="{get_standard_link id='news.php'}?s={$oBlaetterNavi->nAnfang}">{$oBlaetterNavi->nAnfang}</a> ...</li>
                    {/if}
                    {foreach name=blaetternavi from=$oBlaetterNavi->nBlaetterAnzahl_arr item=Blatt}
                        ...
                        {else}
                            <li><a href="{get_standard_link id='news.php'}?s={$Blatt}">{$Blatt}</a></li>
                        {/if}
                    {/foreach}
                    {if $oBlaetterNavi->nEnde != 0}
                        <li> ... <a href="{get_standard_link id='news.php'}?s={$oBlaetterNavi->nEnde}">{$oBlaetterNavi->nEnde}</a></li>
                    {/if}
                    {if $oBlaetterNavi->nAktuelleSeite < $oBlaetterNavi->nSeiten}
                        <li>
                            <a href="{get_standard_link id='news.php'}?s={$oBlaetterNavi->nNaechste}">{lang key="next" section="productOverview"} &raquo;</a>
                        </li>
                    {/if}
                </ul>
            </div>
