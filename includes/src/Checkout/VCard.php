...
class VCard
{
    ...
    {
        $result = [];
        $txtArr = \array_map('\trim', \explode(';', $rawValue));
    ...
    protected function parseVCardLine($line): void
    {
        ...
        $key = \mb_convert_case(\trim(self::unescape($key)), \MB_CASE_LOWER);
        ...
        $rawValue = \trim(self::unescape(\str_replace('-wrap-', '', $rawValue)));
    ...
    public function asKunde(): stdClass
    {
        ...
        foreach ($Kunde as $property => $data) {
            $Kunde->$property = Text::filterXSS($data);
        }
