...
{*
   <form method="post" action="{get_standard_link id='jtl.php'}" name="WunschlisteSuche" class="form">
      ...
      <fieldset>
         ...
         {if $wlsearch}
            <a href="{get_standard_link id='jtl.php'}?wl={$CWunschliste->kWunschliste}" class="wishlistlink">{lang key="wishlistRemoveSearch" section="login"}</a>
         {/if}
      </fieldset>
   </form>
*}
<form method="post" action="{get_standard_link id='jtl.php'}" name="Wunschliste" class="basket_wrapper">
    {$jtl_token}
    {block name="wishlist"}
            ...
            {foreach name=wunschlistepos from=$CWunschliste->CWunschlistePos_arr item=CWunschlistePos}
                <tr>
                    ...
                    <td class="text-right btn-group-vertical">
                        ...
                        {else}
                            <a href="{get_standard_link id='jtl.php'}.php?wl={$CWunschliste->kWunschliste}&wlph={$CWunschlistePos->kWunschlistePos}{if isset($wlsearch)}&wlsearch=1&cSuche={$wlsearch}{/if}" class="btn btn-default" title="{lang key="wishlistaddToCart" section="login"}">
                                <span class="fa fa-shopping-cart"></span>
                            </a>
                        {/if}
                        <a href="{get_standard_link id='jtl.php'}?wl={$CWunschliste->kWunschliste}&wlplo={$CWunschlistePos->kWunschlistePos}{if isset($wlsearch)}&wlsearch=1&cSuche={$wlsearch}{/if}" class="btn btn-default" title="{lang key="wishlistremoveItem" section="login"}">
        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="pull-right btn-group">
                    ...
                    <a href="{get_standard_link id='jtl.php'}?wl={$CWunschliste->kWunschliste}&wlpah=1{if isset($wlsearch)}&wlsearch=1&cSuche={$wlsearch}{/if}" class="btn btn-primary submit">{lang key="wishlistAddAllToCart" section="login"}</a>
                    <a href="{get_standard_link id='jtl.php'}?wl={$CWunschliste->kWunschliste}&wldl=1" class="btn btn-default submit">{lang key="wishlistDelAll" section="login"}</a>
                </div>
            </div>
        </div>
    {/if}
