...
//            }
            ...
            if (isset($post['continue']) && $post['continue'] === '1') {
                ...
                $tab = \Helpers\RequestHelper::verifyGPDataString('tab');
    ...
    public function newsRedirect($tab = '', $msg = '', $urlParams = null): void
    {
        ...
        if (!empty($tab)) {
            ...
            if (isset($tabPageMapping[$tab])
                && \Helpers\RequestHelper::verifyGPCDataInt($tabPageMapping[$tab]) > 1
                && !\array_key_exists($tabPageMapping[$tab], $urlParams)
            ) {
                $urlParams[$tabPageMapping[$tab]] = \Helpers\RequestHelper::verifyGPCDataInt($tabPageMapping[$tab]);
            }
        }
