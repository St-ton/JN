...
class Artikel
{
        ...
        if ($this->kArtikel > 0) {
            ...
            if (is_array($this->oMedienDatei_arr) && count($this->oMedienDatei_arr) > 0) {
                ...
                foreach ($this->oMedienDatei_arr as $i => $oMedienDatei) {
                    ...
                    if (is_array($this->oMedienDatei_arr[$i]->oMedienDateiAttribut_arr) &&
                        count($this->oMedienDatei_arr[$i]->oMedienDateiAttribut_arr) > 0
                    ) {
            ...
            if (is_array($oVariationTMP_arr) && count($oVariationTMP_arr) > 0) {
                ...
                foreach ($oVariationTMP_arr as $i => $oVariationTMP) {
                    ...
                    if ($this->cLagerBeachten === 'Y' &&
                        ...
                        $this->Variationen[$nZaehler]->Werte[$i]->fLagerbestand <= 0 &&
                        $conf['global']['artikeldetails_variationswertlager'] == 2
                    ) {
    ...
    public function fuelleArtikel($kArtikel, $oArtikelOptionen, $kKundengruppe = 0, $kSprache = 0, $noCache = false)
    {
        ...
        if (
            isset($oArtikelOptionen->nVariationDetailPreis) &&
        ...
        // Warenkorbmatrix Variationskinder holen?
        if (
            (isset($oArtikelOptionen->nWarenkorbmatrix) && $oArtikelOptionen->nWarenkorbmatrix == 1) ||
        ...
        $shipping = Shop::DB()->query(
            "SELECT va.kVersandart, IF(vas.fPreis IS NOT NULL, vas.fPreis, va.fPreis) AS minPrice FROM tversandart va
                ...
                WHERE va.cLaender like '%{$countryCode}%'
                    AND (va.cVersandklassen = '-1' OR 
                        (va.cVersandklassen LIKE '% {$this->kVersandklasse} %' 
                        OR va.cVersandklassen LIKE '% {$this->kVersandklasse}'))
                    ...
                    AND (
                        va.kVersandberechnung = 1 
                          OR va.kVersandberechnung = 4
                            OR ( va.kVersandberechnung = 2 
                                AND vas.fBis > 0 
                                AND {$this->fGewicht} <= vas.fBis )
                            OR ( va.kVersandberechnung = 3 
                                AND vas.fBis > 0 
                                AND {$this->Preise->fVKNetto} <= vas.fBis )
                        )
        ...
        foreach ($nEigenschaft_arr as $kEigenschaft => $nEigenschaftWert_arr) {
            ...
            foreach ($oEigenschaft_arr as $oEigenschaft) {
                ...
                if ($this->aufLagerSichtbarkeit($oEigenschaft) &&
                    !in_array($oEigenschaft->kEigenschaftWert, $nPossibleVariation_arr[$oEigenschaft->kEigenschaft])
                ) {
