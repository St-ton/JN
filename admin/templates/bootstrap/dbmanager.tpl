...
{function table_scope_header table=null}
    <h2>{__('table')}: {$table}
        <div class="btn-group btn-group-xs" role="group">
            <a href="{$shopURL}{$route}table={$table}&token={$smarty.session.jtl_token}" class="btn btn-default"><span class="glyphicon glyphicon-equalizer"></span> {__('structure')}</a>
            <a href="{$shopURL}{$route}?select={$table}&token={$smarty.session.jtl_token}" class="btn btn-default"><span class="glyphicon glyphicon-list"></span> {__('show')}</a>
        </div>
    </h2>
{/function}
...
$(function() {
    var new_content = '<form action="{$shopURL}{$route}?command" method="POST">';
    ...
    <div class="row">
            ...
            <nav class="db-sidebar hidden-print hidden-xs hidden-sm">
                <ul class="nav flex-column db-sidenav">
                    {foreach $tables as $table}
                        <li><a href="{$shopURL}{$route}?select={$table@key}&token={$smarty.session.jtl_token}">{$table@key}</a></li>
                    {/foreach}
                </ul>
            </nav>
        </div>
        ...
        <div class="col-md-10">
            <ol class="simple-menu">
                <li><a href="{$shopURL}{$route}">{__('overview')}</a></li>
                <li><a href="{$shopURL}{$route}?token={$smarty.session.jtl_token}&command"><span class="glyphicon glyphicon-flash"></span> {__('sqlCommand')}</a></li>
            ...
            {if $sub === 'command'}
                ...
                <form action="{$shopURL}{$route}?command" method="POST">
                    {$jtl_token}
                    <div class="form-group">
                        ...
                        <table class="table table-striped table-condensed table-bordered table-hover table-sticky-header">
                            ...
                            {foreach $tables as $table}
                                <tr class="text-vcenter{if count($definedTables) > 0 && !($table@key|in_array:$definedTables || $table@key|substr:0:8 === 'xplugin_')} warning{/if}" id="table-{$table@key}">
                                    <td><a href="{$shopURL}{$route}?select={$table@key}&token={$smarty.session.jtl_token}">{$table@key}</a></td>
                                    <td class="text-center">
                                        <div class="btn-group btn-group-xs" role="group">
                                            <a href="{$shopURL}{$route}?table={$table@key}&token={$smarty.session.jtl_token}" class="btn btn-default"><span class="glyphicon glyphicon-equalizer"></span> {__('structure')}</a>
                                            <a href="{$shopURL}{$route}?select={$table@key}&token={$smarty.session.jtl_token}" class="btn btn-default"><span class="glyphicon glyphicon-list"></span> {__('show')}</a>
                                        </div>
                                    </td>
            ...
            {elseif $sub === 'select'}
                ...
                <div id="filter">
                    <form method="GET" action="{$shopURL}{$route}" data-sql={$info.statement|json_encode}>
                ...
                <div class="query">
                    ...
                    <div class="query-sub">
                        ...
                        <a href="{$shopURL}{$route}?command&query={$info.statement|urlencode}&token={$smarty.session.jtl_token}">
