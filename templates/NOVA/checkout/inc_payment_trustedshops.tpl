 ...
 *}
{block name='checkout-inc-payment-trustedshops'}
    {if isset($oTrustedShops->oKaeuferschutzProdukte->item) && $oTrustedShops->oKaeuferschutzProdukte->item|@count > 0 && $Einstellungen.trustedshops.trustedshops_nutzen === 'Y'}
        <div id="ts-buyerprotection">
            {row}
                {block name='checkout-inc-payment-trustedshops-main-content'}
                    {col cols=10}
                    ...
                    {/col}
                {/block}
                {block name='checkout-inc-payment-trustedshops-link'}
                    {col cols=2}
