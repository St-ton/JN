        ...
        {container}
            {row class="bg-info"}
                {foreach $smarty.session.Warenkorb->PositionenArr as $oPosition}
                    <div class="d-flex w-100 px-2{if $oPosition@first} py-4{else} pb-4{/if}">
                    {if !$oPosition->istKonfigKind()}
                        {if $oPosition->nPosTyp == C_WARENKORBPOS_TYP_ARTIKEL}
                            {col md=3}
                                {image src=$oPosition->Artikel->Bilder[0]->cURLMini alt=$oPosition->Artikel->cName class="img-fluid"}
                            {/col}
                            {col md=6}
