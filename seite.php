...
if ($link->getLinkType() === LINKTYP_STARTSEITE) {
    ...
    $liveSearchTop  = CMS::getLiveSearchTop($conf, $languageID);
    $liveSearchLast = CMS::getLiveSearchLast($conf, $languageID);
    ...
    $smarty->assign('oHersteller_arr', Hersteller::getAll(true, $languageID));
} elseif ($link->getLinkType() === LINKTYP_NEWSLETTERARCHIV) {
    $smarty->assign('oNewsletterHistory_arr', CMS::getNewsletterHistory($languageID));
    ...
    if ($conf['sonstiges']['sonstiges_gratisgeschenk_nutzen'] === 'Y') {
        $freeGifts = CMS::getFreeGifts($conf, $languageID, $cgroupID);
