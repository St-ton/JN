...
<div class="stepwizard" id="checkout_steps">
    <ol class="list-unstyled stepwizard-row row">
        <li class="col-md-4 stepwizard-step step2 state{$bestellschritt[1]} first{if $bestellschritt[1]==3} text-muted{/if}">
            {if $bestellschritt[1]==2}<a href="bestellvorgang.php?editRechnungsadresse=1">{/if}
            <span {if $bestellschritt[1]==1 || $bestellschritt[2]==1}class="btn btn-circle btn-primary"{else}class="btn btn-circle btn-default"{if $bestellschritt[1]!=2} disabled{/if}{/if}>1</span><div class="step-name hidden-xs">{lang key="billingAdress" section="checkout"} & {lang key="shippingAdress" section="checkout"}</div>
        ...
        *}
        <li class="col-md-4 stepwizard-step step4 state{$bestellschritt[3]}{if $bestellschritt[3]==3} text-muted{/if}">
            {if $bestellschritt[3]==2}<a href="bestellvorgang.php?editVersandart=1">{/if}
            <span {if $bestellschritt[3]==1}class="btn btn-circle btn-primary"{else}class="btn btn-circle btn-default"{if $bestellschritt[3]!=2} disabled{/if}{/if}>3</span><div class="step-name hidden-xs">{lang key="shipmentMode" section="checkout"} & {lang key="paymentMethod" section="checkout"}</div>
        ...
        *}
        <li class="col-md-4 stepwizard-step step6 state{$bestellschritt[5]}{if $bestellschritt[5]==3} text-muted{/if}">
