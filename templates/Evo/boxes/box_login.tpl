<section class="panel panel-default box box-login" id="sidebox{$oBox->kBox}">
    ...
    <div class="panel-body">
        ...
        {else}
            <a href="{get_static_route id='jtl.php'}" class="btn btn-default btn-block btn-sm btn-account">{lang key="myAccount" section="global"}</a>
            <a href="{get_static_route id='jtl.php'}?logout=1&token={$smarty.session.jtl_token}" class="btn btn-block btn-sm btn-warning btn-logout">{lang key="logOut" section="global"}</a>
        {/if}
    </div>
</section>