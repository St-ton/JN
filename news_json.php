...
$xEinstellung_arr = Shop::getSettings([CONF_GLOBAL]);
$nLimit           = (isset($_REQUEST['limit']) && intval($_REQUEST['limit']) > 0)
    ? (int)$_REQUEST['limit']
    : 100;
...
if ($oNews_arr !== null) {
    if (isset($_REQUEST['notimeline']) && intval($_REQUEST['notimeline']) === 1) {
        ...
        $oNewsJson = new NewsJson(
            "{$xEinstellung_arr['global_shopname']} News",
            "Es war einmal...",
            date('Y,m,d'),
            $oNews_arr
        );
