...
class UpdateTestDB
{
    ...
    {
        $updater = new UpdaterForTestEnvironment($this->db);
        $sessionDummy = [];
        try {
            $disabledPlugins = 0;
            ...
            if ($dbVersion->getMajor() === 4) {
                ...
                if ($disabledPlugins > 0) {
                    $sessionDummy['disabledPlugins'] = $disabledPlugins;
                }
            }
            $updateResult    = $updater->update(true);
            $availableUpdate = $updater->hasPendingUpdates();
            if ($updateResult instanceof IMigration) {
                $updateResultResponse = \sprintf('Migration: %s', $updateResult->getDescription());
            } elseif ($updateResult instanceof Version) {
                $updateResultResponse = \sprintf('Version: %s', $updateResult->__toString());
            } else {
                $updateResultResponse = \sprintf('Version: %.2f', (float)$updateResult / 100);
            }
