...
class ReviewReminder
{
     ...
     * @throws \Exceptions\EmptyResultSetException
     */
    private function fireSQL(): void
    {
        $sqlString    = 'SELECT kBestellung
            ...
            WHERE dVersandDatum IS NOT NULL
                ...
                ' . $this->sqlPartBundle2;
        $this->orders = Shop::Container()->getDB()->query($sqlString, \DB\ReturnType::ARRAY_OF_OBJECTS);
        if (count($this->orders) === 0) {
            throw new \Exceptions\EmptyResultSetException('Keine Bestellungen für Bewertungserinnerungen gefunden.');
        }
    }
    ...
    private function getCustomerGroups(): void
    {
        ...
        if (empty($this->sqlPartCustomerGroups)) {
            throw new \Exceptions\EmptyResultSetException('Keine Kundengruppe für Bewertungserinnerungen gefunden');
        }
    }
    ...
    private function calculateSentTime(): void
    {
        ...
        if ($this->shippingDays <= 0) {
            throw new \Exceptions\InvalidSettingException('Einstellung bewertungserinnerung_versandtage ist 0');
        }
