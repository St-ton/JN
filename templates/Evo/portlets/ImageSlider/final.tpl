<div {$instance->getAttributeString()}>
    {if $instance->getProperty('slides') > 0}
        <div class="slider-wrapper theme-{$instance->getProperty('slider-theme')}">
                ...
                {foreach from=$instance->getProperty('slides') item=slide}
                    ...
                    {if !empty($slide.url)}
                        {if !empty($slide['target-url'])}
                            <a href="{$slide['target-url']}"
                               {if !empty($slide.cTitle)}title="{$slide.cTitle}"{/if}
                               class="slide">
                        {else}
                            <div class="slide">
                        {/if}
            ...
            {foreach $instance->getProperty('slides') as $slide}
                    ...
                    slider.nivoSlider({
                        ...
                        beforeChange: function ()
                        {
                            ...
                            setTimeout(function () {
                                ...
                                var nivoHeight = $('#{$instance->getProperty("uid")} ').height();
                                ...
                                $('#{$instance->getProperty("uid")} .nivo-slice img')
                                    .css('left', xStart)
                                    .css('top', yStart)
                                    .animate(
                                ...
                                $('#{$instance->getProperty("uid")} .nivo-main-image')
                        ...
                        afterLoad: function ()
                        {
                            ...
                            setTimeout(function () {
                                $('#{$instance->getProperty("uid")} , .nivo-control').animate({ opacity: 1 }, animSpeed);
                            }, 0);
                            ...
                            $('#{$instance->getProperty("uid")} .nivo-prevNav, #{$instance->getProperty("uid")} .nivo-nextNav')
                                .on('click', function () {
                                    setTimeout(function () {
                                        $('#{$instance->getProperty("uid")} .nivo-main-image').css('opacity', 0);
                                    }, 20);
