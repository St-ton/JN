<li class="list-group-item boxRow">
    ...
    <div class="col-xs-2">
        ...
        <a href="boxen.php?action=del&page={$nPage}&position={$position}&item={$oBox->kBox}&token={$smarty.session.jtl_token}"
           onclick="return confirmDelete('{$oBox->cTitel}');" title="Aus allen Seiten entfernen">
            <i class="fa fa-lg fa-trash"></i>
        </a>
        {if $nPage == 0}
            <a href="#" title="Sichtbar auf folgenden Seiten: {$oBox->cVisibleOn}">
                <i class="fa fa-lg fa-info"></i>
            </a>
        {/if}
        {if $nPage == 1 || $nPage == 2 || $nPage == 24 || $nPage == 31}
            <a href="#" data-filter="box-filter-{$oBox->kBox}" data-box-id="{$oBox->kBox}"
               data-box-title="{$oBox->cTitel}" class="box-filter" data-toggle="modal" data-target="#boxFilterModal">
