    ...
    if (count($articles) > 0) {
        ...
        foreach ($articles as $article) {
            if (isset($article['kArtikel'])) {
                if ($smarty !== null) {
                    $smarty->clearCache(null, CACHING_GROUP_ARTICLE . '|id' . (int)$article['kArtikel']);
                }
                ...
                if ($article['kHersteller'] > 0) {
                    ...
                    if ($smarty !== null) {
                        $smarty->clearCache(null, CACHING_GROUP_MANUFACTURER . '|id' . (int)$article['kHersteller']);
                    }
                }
                foreach ($article['categories'] as $category) {
                    $cacheTags[] = CACHING_GROUP_CATEGORY . '_' . (int)$category->kKategorie;
                    if ($smarty !== null) {
                        $smarty->clearCache(null, CACHING_GROUP_CATEGORY . '|id' . (int)$category->kKategorie);
                    }
                }
                ...
                foreach ($parentIDs as $parentID) {
                    ...
                    if ($smarty !== null) {
                        $smarty->clearCache(null, CACHING_GROUP_ARTICLE . '|id' . (int)$parentID);
                    }
                }
                $cacheTags[] = CACHING_GROUP_ARTICLE . '_' . (int)$article;
                $deps     [] = (int)$article;
                if ($smarty !== null) {
                    $smarty->clearCache(null, CACHING_GROUP_ARTICLE . '|id' . (int)$article);
                }
            }
        }
        ...
        if (count($deps) > 0) {
            ...
            foreach ($manufacturers as $manufacturer) {
                ...
                if ($smarty !== null) {
                    $smarty->clearCache(null, CACHING_GROUP_MANUFACTURER . '|id' . (int)$manufacturer->kHersteller);
                }
            }
            ...
            foreach ($parentArticles as $parentArticle) {
                ...
                if ($smarty !== null) {
                    $smarty->clearCache(null, CACHING_GROUP_ARTICLE . '|id' . (int)$parentArticle->id);
                }
            }
        }
