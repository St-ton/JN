...
if (isset($_POST['keywording']) && (int)$_POST['keywording'] === 1 && validateToken()) {
    foreach ($sprachen as $sprache) {
        ...
        Shop::Container()->getDB()->delete('texcludekeywords', 'cISOSprache', $text->cISOSprache);
        Shop::Container()->getDB()->insert('texcludekeywords', $text);
    }
    Shop::Cache()->flushTags([CACHING_GROUP_OPTION]);
}
$keywords = [];
foreach ($sprachen as $sprache) {
    $text                     = Shop::Container()->getDB()->select('texcludekeywords', 'cISOSprache', $sprache->cISO);
