...
class FilterItemPriceRange extends AbstractFilter
{
    ...
    public function getSQLJoin()
    {
        ...
        $res[] = (new FilterJoin())->setComment('join1 from FilterItemPriceRange')
                                   ->setType('JOIN')
                                   ->setTable('tpreise')
        ...
        $res[] = (new FilterJoin())->setComment('join4 from FilterItemPriceRange')
                                   ->setType('LEFT JOIN')
    ...
    public function getOptions($mixed = null)
    {
        ...
        // Automatisch
        if ($this->getConfig()['navigationsfilter']['preisspannenfilter_anzeige_berechnung'] === 'A') {
            $state->joins[] = (new FilterJoin())->setComment('join1 from FilterItemPriceRange::getOptions()')
                                                ->setTable('tpreise')
                                                ->setType('JOIN')
            ...
            if (isset($oPreisspannenFilterMaxMin->fMax) && $oPreisspannenFilterMaxMin->fMax > 0) {
                ...
                for ($i = 0; $i < $nAnzahlSpannen; ++$i) {
                    $oPreisspannenFilter       = new FilterExtra();
                    ...
                    $oPreisspannenFilter->setType($this->getType())
                                        ->setClassName($this->getClassName())
                                        ->setParam($this->getUrlParam())
                                        ->setName($oPreisspannenFilter->cVonLocalized . ' - ' . $oPreisspannenFilter->cBisLocalized)
                                        ->setValue($i)
                    ...
                    $oPreisspannenFilter->nAnzahlArtikel = $oPreisspannenFilter->getCount();
...
//                    );
            ...
            if (is_array($oPreisspannenfilter_arr) && count($oPreisspannenfilter_arr) > 0) {
                ...
                Shop::dbg($oPreisspannenfilter_arr, true, '$oPreisspannenfilter');
                foreach ($oPreisspannenfilter_arr as $i => $oPreisspannenfilter) {
                    $priceRange = (new FilterExtra())
                    ...
                    $priceRange->nVon           = $oPreisspannenfilter->nVon;
                    $priceRange->nBis           = $oPreisspannenfilter->nBis;
                    $priceRange->nAnzahlArtikel = $priceRange->getCount();
                    // Localize Preise
