...
zip -r -q ${ARCHIVE_PATH} . -x \*.git* \*.idea* \*build/* \*docs/* \*patch-dir-* \*templates/Evo/\* \*templates_c/*.php\* \*templates_c/*/\* \*tests/* \*.asset_cs \*.php_cs \*.travis.yml \*phpunit.xml;
echo "  ${FILENAME}";
echo "";
...
if [[ ! -z $(find . -maxdepth 1 -type d -regex '^./patch-dir-.*') ]]; then
    ...
    do
        ...
        cd ${path};
        zip -r -q ${PATCH_ARCHIVE_PATH} . -x  \*.git* \*.idea* \*build/* \*docs/* \*patch-dir-* \*templates/Evo/\* \*templates_c/*.php\* \*templates_c/*/\* \*tests/* \*.asset_cs \*.php_cs \*.travis.yml \*phpunit.xml;
        echo "    ${PATCH_FILENAME}";
        cd ${CUR_PWD};
